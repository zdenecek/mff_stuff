...
int udp_server()
{
  openlog ("udpserverlog", LOG_CONS | LOG_PID | LOG_NDELAY | LOG_PERROR, LOG_USER);



  cout << "Welcome to txhe UDP server" << endl;
  int listenSocket, i;
  ...
  while (1) {
    ...
    syslog (LOG_NOTICE, "Client connected %s",  inet_ntoa(clientAddress.sin_addr));
    ...
    syslog (LOG_NOTICE, "Message length %ld",  strlen(line));
