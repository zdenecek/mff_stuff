...
Button led_inc_button = Button(button3_pin);
...
Button buttons[] = {inc_button, dec_button, led_inc_button};
...
//  unsigned long time = millis();
  ...
  if (dec_button.pressed(time))
  {
    state.decrement(current_order);
  }
  if (inc_button.pressed(time))
  {
    state.increment(current_order);
  }

  if (led_inc_button.pressed(time))
  {
    current_order++;
