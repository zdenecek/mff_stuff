...
def main(args: argparse.Namespace) -> list[float]:
    ...
    xsn = np.ones(args.data_size)

    data, target = np.empty([args.data_size, 0]), ys
    ...
    for order in range(1, args.range + 1):
        ...
        xsn *= xs
        data = np.insert(data, order - 1, xsn, axis=1)

        # TODO: Split the data into a train set and a test set.
        # Use `sklearn.model_selection.train_test_split` method call, passing
        # arguments `test_size=args.test_size, random_state=args.seed`.

        train_data, test_data, train_target, test_target = sklearn.model_selection.train_test_split(data,
                                                                                                    target,
        ...
        rmse = sklearn.metrics.mean_squared_error(test_target, predicted_target, squared=False)
