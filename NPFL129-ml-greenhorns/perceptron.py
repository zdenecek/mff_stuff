...
def main(args: argparse.Namespace) -> np.ndarray:
    ...
    while not done:
        ...
        perm_data = np.copy(data[permutation,])
        perm_target = np.copy(target[permutation])

        done = True
        for i, feature in enumerate(perm_data):
            y = feature @ weights
            if perm_target[i] * y <= 0:
                done = False
                weights = weights + perm_target[i] * feature
